...
export const JobCard = ({ vacancy }: Props) => {
  ...
  return (
    <Card shadow="sm" padding="lg" radius="md" withBorder mb="sm">
      <Stack gap="xs">
        <Text fw={600} size="lg" c="blue">{vacancy.name}</Text>
        ...
        <Group gap="xs">
          ...
          <Badge variant="light" color="gray">{vacancy.schedule?.name}</Badge>
        </Group>

        <Text c="dimmed" size="sm">{vacancy.area?.name}</Text>

        <Group mt="md">
          <Button variant="default" color="gray">Смотреть вакансию</Button>
          <Button component="a" href={vacancy.alternate_url} target="_blank" color="blue">
            Откликнуться
          </Button>
        </Group>
      </Stack>
